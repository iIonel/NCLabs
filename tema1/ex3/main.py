...
class T:
    t_calcul = [
        ...
        lambda a: (10395 * a - 1260 * (a ** 3) + 21 * (a ** 5)) / (10395 - 4725 * (a ** 2) + 210 * (a ** 4) - (a ** 6)), #t6
...
def aprox():
    with open("results.txt", "w") as file:
        ...
        test_numbers = generation()

        file.write(f"random [-pi/2, pi/2]: {test_numbers}")
        file.write("\n")

        for number in test_numbers:
            max_n = 6
            t_aprox = [(i + 4,float(abs(float(T.t_calcul[i](number)) - float(math.tan(number))))) for i in range(0,max_n)]
